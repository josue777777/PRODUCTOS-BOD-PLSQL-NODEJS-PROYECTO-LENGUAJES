...
-- 3. procedimiento: seleccionar siguiente peticiÛn pendiente (bloquea la fila)
...
BEGIN
    ...
    IF p_id_peticion IS NOT NULL THEN
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                p_mensaje := 'PeticiÛn inv·lida o ya atendida (id='||p_id_peticion||')';
                RETURN;
        END;
    END IF;
    ...
    -- 4.11. cerrar peticiÛn si vino desde la bandeja
    IF p_id_peticion IS NOT NULL THEN
...
-- 7.2 individual (si querÈs disparar una especÌfica por id)
SET SERVEROUTPUT ON;
DECLARE
  v_msg VARCHAR2(4000);
BEGIN
